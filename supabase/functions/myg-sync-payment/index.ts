...
type SyncInput = { gateway_payment_id: string }
...
function isPaid(chargesPayload: any): boolean {
  const charges =
...
function sanitizeGatewayPayload(payload: any): any {
  ...
  if (payload && typeof payload === 'object') {
    if (Array.isArray((payload as any).data)) {
      return { ...payload, data: (payload as any).data.map(sanitizeCharge) }
    }
    if (Array.isArray((payload as any).charges)) {
      return { ...payload, charges: (payload as any).charges.map(sanitizeCharge) }
    }
  }
  return payload
}
...
serve(async (req) => {
  try {
    ...
    const gateway_payment_id = String(body?.gateway_payment_id || '').trim()
    if (!gateway_payment_id) return json({ error: 'missing_parameters' }, 400)
    ...
    if (!payment) return json({ ok: true, found: false }, 200)
    if (String(payment.user_id) !== String(callerUserId)) return json({ error: 'forbidden' }, 403)

    if (String(payment.status || '').toLowerCase() === 'paid') {
      return json({ ok: true, found: true, status: 'paid' }, 200)
    }

    const token = await getMygAuthToken()
    if (!token || !MYG_API_KEY) return json({ error: 'gateway_auth_failed' }, 502)

    const url = `${MYG_BASE_URL}/payments/v1/paymentlink/charges/${encodeURIComponent(gateway_payment_id)}`
    const res = await fetch(url, {
      method: 'GET',
      headers: { 'x-api-key': MYG_API_KEY, 'Accept': 'application/json', 'Authorization': token },
    })
    const ct = res.headers.get('Content-Type') || ''
    const payload = ct.includes('application/json') ? await res.json().catch(() => ({})) : await res.text().catch(() => '')

    if (!res.ok) {
      await supabaseAdmin.from('payments').update({ raw_payload: payload }).eq('id', payment.id)
      return json({ error: 'gateway_error', status: res.status, payload }, 502)
    }

    const classification = classifyStatus(payload)
    const sanitized = sanitizeGatewayPayload(payload)
    if (!classification.paid && classification.status !== 'failed' && classification.status !== 'canceled') {
      await supabaseAdmin.from('payments').update({ raw_payload: sanitized }).eq('id', payment.id)
      return json({ ok: true, found: true, status: String(payment.status || 'pending'), paid: false }, 200)
    }
    ...
    if (classification.status === 'paid') {
      await supabaseAdmin
        .from('payments')
        .update({ status: 'paid', paid_at: now, raw_payload: sanitized })
        .eq('id', payment.id)
      await supabaseAdmin
        .from('profiles')
        .upsert({ user_id: payment.user_id, active_plan: payment.plan_slug, plan_activated_at: now }, { onConflict: 'user_id' })
      return json({ ok: true, found: true, status: 'paid', paid: true }, 200)
    }

    if (classification.status === 'failed' || classification.status === 'canceled') {
      await supabaseAdmin
        .from('payments')
        .update({ status: classification.status, raw_payload: sanitized })
        .eq('id', payment.id)
      return json({ ok: true, found: true, status: classification.status, paid: false }, 200)
    }
