...
export default async function handler(req, res) {
  try {
    ...
    if (!code) {
      ...
      const errCode = String(url.searchParams.get('error_code') || '').trim()
      const errDesc = String(url.searchParams.get('error_description') || '').trim()
      res.setHeader('Set-Cookie', stateClearCookie ? [stateClearCookie, legacyClearCookie] : legacyClearCookie)
      res.statusCode = 302
      if (err || errCode || errDesc) {
        const target = new URL(`${appOrigin}/login`)
